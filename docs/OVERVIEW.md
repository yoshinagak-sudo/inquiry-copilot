...
flowchart LR
    ...
    B --> H[📝 Gmail に下書きとして<br/>自動セット]
    H --> C[👤 担当者は普段の Gmail で<br/>下書きを開いて編集・送信]
    C -. 草案と送信内容の<br/>差分が大きい場合 .-> E[💡 ナレッジ候補を<br/>自動提案]
    E --> F[👤 管理者が<br/>承認 / 却下]
    F -. 承認分のみ昇格 .-> G[(📚 ナレッジベース)]
    G --> B
```

- **担当者は普段の Gmail のままご利用いただけます。** AI が生成した返信草案は **Gmail の「下書き」として自動セット** されるため、担当者は本システムにログインせず、Gmail の下書きを開いて編集・送信するだけで業務が完結します。
- **Gmail と直接連携します。** OAuth 認証を一度行うだけで、受信箱の取り込みと下書きの作成が御社の Gmail アカウント上で完結します。
...
### ②-A. 普段の Gmail で下書きを開いて送信する（推奨）

問い合わせを取り込むと、Gmail 上の **「下書き」フォルダ** に AI が組み立てた返信案が自動的にセットされます。
担当者は本システムにログインしなくとも、**普段の Gmail を開いて下書きをクリックするだけ** で内容を確認・編集・送信できます。スレッドにも紐づくため、元メールの隣に下書きが並ぶ Gmail 標準の操作感のままご利用いただけます。
...
本システム上での編集を選んだ場合は、テキストエリアで自由に編集できます。
編集が大きく必要だと感じた場合は、「再生成」機能で別の草案を取得することも可能です。
編集を終えたら **送信ボタン** を押すだけ。30 秒のカウントダウンが開始され、その間はいつでもキャンセル可能ですので、誤送信のご心配はございません。

詳細画面には **「📝 Gmail で開いて編集」ボタン** も用意されており、本システム上で確認した後に Gmail 側に切り替えて編集することも可能です。
